...
std::pair<std::size_t, std::size_t> ManipulatorGridCSpace2D::getCellFromPoint(double x0, double x1) const {
...
std::unique_ptr<amp::GridCSpace2D> MyManipulatorCSConstructor::construct(const amp::LinkManipulator2D& manipulator, const amp::Environment2D& env) {
    ...
    std::unique_ptr<ManipulatorGridCSpace2D> cspace_ptr = std::make_unique<ManipulatorGridCSpace2D>(m_cells_per_dim, m_cells_per_dim, 0, 2 * M_PI, 0, 2 * M_PI);
    // In order to use the pointer as a regular GridCSpace2D object, we can just create a reference
    ManipulatorGridCSpace2D& cspace = *cspace_ptr;
